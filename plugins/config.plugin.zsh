...
source ${0:h}/fbtools/init.zsh
source ${0:h}/helpers/init.zsh

 command -v pyenv &> /dev/null && eval "$(pyenv init -)"
 8   │ command -v pyenv-virtualenv-init &> /dev/null && eval "$(pyenv virtualenv-init -)"
...
if [[ ${MYPROMPT:=spaceship} = dolphin ]]; then
    isdolphin=true
