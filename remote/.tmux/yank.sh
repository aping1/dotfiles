...
if [[ ! ${copy_backend} ]] && "${copy_backend_remote_tunnel_port}" ]] && [ "$(ss -n -4 state listening "( sport = $copy_backend_remote_tunnel_port )" | tail -n +2 | wc -l)" -eq 1 ]; then
...
function ocs52_copy() {
    ...
    esc="\033]52;c;$( printf %s "${*}" | head -c $maxlen | base64 | tr -d '\r\n' )\a"
